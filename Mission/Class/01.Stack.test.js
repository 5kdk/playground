...
describe('Stack', () => {
  describe('constructor', () => {
    ...
    test('인수로 배열 이외의 값을 전달하면 에러를 발생시킨다.', () => {
      ...
      expect(() => new Stack(true)).toThrow(new TypeError('true is not an array'));
      expect(() => new Stack({})).toThrow(new TypeError('[object Object] is not an array'));
    });
  });
  ...
  describe('Stack.from', () => {
    ...
    test('인수로 배열 이외의 값을 전달하면 에러를 발생시킨다.', () => {
      ...
      expect(() => Stack.from(true)).toThrow(new TypeError('true is not an array'));
      expect(() => Stack.from({})).toThrow(new TypeError('[object Object] is not an array'));
    });
  });
  ...
  describe('Stack.prototype.size', () => {
    ...
    test('strict mode에서 size에 값을 할당하면 에러가 발생한다.', () => {
      expect(() => {
        Stack.from().size = 100;
      }).toThrow(new TypeError('Cannot set property size of #<Stack> which has only a getter'));
    });
  });
  ...
  describe('Stack.prototype.push', () => {
    test('stack의 가장 후미에 요소를 추가하고 변경된 stack을 반환한다.', () => {
      ...
      stack.push(4).push('a').push().push(undefined).push(null);
      expect([...stack]).toEqual([1, 2, 3, 4, 'a', undefined, null]);
    });
  ...
  describe('Stack.prototype.clone', () => {
    ...
    test('원본 stack이 변경되어도 복사본 stack은 변경되지 않는다.', () => {
      expect(stack.push(1)).not.toEqual(clone);
    });
  });
